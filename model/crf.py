...
class CRF(nn.Module):
    ...
    def _calculate_PZ(self, feats, mask):
        """
        计算序列得分
        这个函数，只是根据 随机的transitions ，前向传播算出的一个score
            input:
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        # print 'partition_history=', partition_history, seq_len, batch_size
        # print 'torch.cat(partition_history, 0)=', torch.cat(partition_history, 0)
        # print 'torch.cat(partition_history, 0)===', torch.cat(partition_history, 0).view(seq_len, batch_size, -1)
